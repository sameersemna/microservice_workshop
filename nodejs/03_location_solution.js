    ...
    var queue = connection.queue('needs', config.queue, function __queueReady(queue) {
        ...
        queue.subscribe(function __listener(need) {
          connection.queue('solutions', config.queue, function __queueReady(solutionQueue) {
            exchange.publish('solutions', SolutionPackage.create(need.uuid, 'Location', {'price': 100, 'location': 'Miami', 'brand': 'Toyota'}));
            exchange.publish('solutions', SolutionPackage.create(need.uuid, 'Location', {'price': 99, 'location': 'London', 'brand': 'Honda'}));
          });
        });
    });
  });
